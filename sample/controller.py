...
class Persona:
    ...
    def __init__(self, nombres: str, apellidos: str):
        if functions.es_vacio_o_nulo(nombres) is False:
            ...
            nombres = "".join([caracter if caracter.isalpha()
                              is True else " " for caracter in nombres])
        ...
        if functions.es_vacio_o_nulo(apellidos) is False:
            ...
            apellidos = "".join(
                [caracter if caracter.isalpha() is True else " " for caracter in apellidos])
...
class Institucion:
    ...
    def get_codigo(self, id_institucion: int):
        ...
        self.codigo_institucion = next(
            (id[1] for id in self.list_instituciones if id[0] == self.id_institucion), None)
...
class Cargo:
    ...
    def licitacion_relacionada_dentro_de_texto(self, texto: str):
        while True:
            identificador = functions.buscar_identificador_licitacion_en_texto(
                texto)
...
class Entidad:
    ...
    def __init__(self, rut: str, nombre: str, giro: str, domicilio: str, representante_directorio: str, naturaleza: str, directorio: str):
        if functions.es_vacio_o_nulo(rut) is False and len(rut) >= 5:
            ...
            if len(rut) >= 9 and len(rut) <= 10 and rut[-2:-1] == '-' and rut[:-2].isnumeric() is True:
                self.rut = functions.dar_formato_al_rut(
                    rut)  # Formato para el rut chileno
        ...
        if functions.es_vacio_o_nulo(nombre) is False and any(char.isalpha() for char in nombre) and len(nombre) >= 3:
            ...
            nombre = functions.limpiar_nombre(nombre).replace(
                " SOCIEDAD ANONIMA", "").replace("LTDA", "").replace(" LTD", "")
            self.nombre = " ".join(
                [palabra for palabra in nombre.split() if palabra not in ["SA", "LIMITADA"]])
        ...
        if functions.es_vacio_o_nulo(representante_directorio) is False and any(char.isalpha() for char in representante_directorio):
            representante_directorio = functions.limpiar_texto(
                representante_directorio)
            self.representante_directorio = functions.quitar_puntos(
                representante_directorio).title()
...
class HoraChile():
    ...
    def __init__(self):
        ...
        self.__utc_chile = tz.gettz('America/Santiago')
        ...
        self._hora_inicio_extraccion = datetime_chile.replace(hour=18, minute=00, second=0, microsecond=0)+datetime.timedelta(days=-1 if datetime_chile.time() <= datetime.time(
            8, 0, 0, 0) and datetime_chile.time() >= datetime.time.min else 0)  # Hora Oficial Entre 22:00 a 07:00 hrs. Restriccion horaria para extraer desde la API
        self._hora_final_extraccion = datetime_chile.replace(hour=8, minute=00, second=0, microsecond=0)+datetime.timedelta(
            days=1 if datetime_chile.time() >= datetime.time(8, 0, 0, 1) and datetime_chile.time() <= datetime.time.max else 0)  # hora Oficial
