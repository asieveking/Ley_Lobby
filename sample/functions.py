...
def limpiar_texto(texto: str) -> str:
    ...
    texto = texto.replace('--', ' ').replace(': -', ':-').replace('**', ' ').replace(
        '""', '"').replace('.,', '.').replace('Nº', 'Nº ').replace('/ ', '/').replace(' /', '/')
    texto = " ".join(texto.split())
...
# TODO Este metodo puede ser mejorado a uno por expresion regular
def stringafloat(texto: str) -> float:
    texto = texto.replace("UF", "").replace("$", "").replace(
        "US", "").replace("D", "").replace("EUR", "")
...
def email_es_valido(email: str) -> str:
    try:
        re.search(
            "^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$|[\w]+\@[a-zA-Z0-9]+\.[a-zA-Z]{2,3}|^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$", email).group(0)
        return True
    except:
        return False
...
# 5 segundos de espera por defecto. Si disminuye este valor a menos de 5, la consulta falla.
def get_request_api(url: str, time_start: time, header: dict = {}, verify: bool = True, segundos_espera: int = 2) -> list:
    flag_loop = True
    while flag_loop:
        diferencia = segundos_espera-(time.time()-time_start)
        # 300 miliseconds (.3) or 5 seconds (5)wwwwwwww
        time.sleep(0 if diferencia < 0 else diferencia)
        #payload,headers ={},{}
