...
def main():
    ...
    if obj_hora_chile.calcular_si_hora_de_extraccion_es_valida():  # is False:
        diferencia_entre_hora_inicio_extraccion_y_hora_chile = obj_hora_chile._hora_inicio_extraccion - \
            obj_hora_chile.reconstruir_hora_de_chile()
        print(
            f'Precaucion⚠: el programa se debe ejecutar entre las: {obj_hora_chile._hora_inicio_extraccion.time()} hasta las {obj_hora_chile._hora_final_extraccion.time()}, Horario de Chile')
        time.sleep(
            diferencia_entre_hora_inicio_extraccion_y_hora_chile.total_seconds()+1)
    ...
    try:
        ...
        # Se quita 2 segs para comenzar lo mas pronto posible con la primera request a la API
        time_start = time.time()-2
        header = {'Api-Key': api_key, 'content-type': 'application/json'}

        url = "https://www.leylobby.gob.cl/api/v1/"
        elements_url = ["audiencias?page=", "audiencias/",
                        "cargos-pasivos/", "instituciones/"]

        with sql_connection.SQLServer() as crsr:
            # List instituciones
            crsr.execute(
                'SELECT Id_Institucion, Id_Codigo FROM Ley_Lobby.dbo.Institucion')
            ...
            list_num_page = crsr.execute(
                statement, ["Audiencia"]).fetchall()[0]
            ...
            url_api_list_audiencias_page, url_api_audiencia, url_api_list_cargos_pasivos, url_api_list_instituciones = [
                url+element+'{}' for element in elements_url]

            while obj_hora_chile.calcular_si_fecha_actual_es_mayor_a_la_hora_final_de_extraccion() == False:
                list_audiencias_api, time_start, flag = functions.get_request_api(
                    url_api_list_audiencias_page.format(obj_num_page.num_page), time_start, header)
                ...
                for audiencia in list_audiencias_api["data"]:
                    ...
                    # Load Persona Pasiva
                    # (self,nombres,apellidos): audiencia["id_sujeto_pasivo"]
                    obj_persona_pasiva = controller.Persona(
                        audiencia['nombres'], audiencia['apellidos'])
                    ...
                    crsr.execute(
                        statement, (obj_persona_pasiva._id_sujeto_pasivo_api))
                    list_cargos_pasivos_db = []
                    list_cargos_pasivos_db = crsr.fetchall()

                    # Get Cargos Pasivos API
                    list_cargos_pasivos_api, time_start, flag = functions.get_request_api(
                        url_api_list_cargos_pasivos.format(obj_persona_pasiva._id_sujeto_pasivo_api), time_start, header)
                    cantidad_total_de_peticiones_establecidos_en_la_API -= 1
                    ...
                    for cargo_pasivo in list_cargos_pasivos_api:
                        ...
                        obj_cargo = controller.Cargo(
                            cargo_pasivo["id_cargo_pasivo"], cargo_pasivo["cargo"])
                        obj_cargo.rellenar_campos(
                            cargo_pasivo["resolucion"], cargo_pasivo["resolucion_url"], cargo_pasivo["fecha_inicio"], cargo_pasivo["fecha_termino"])

                        # Load Institucion
                        obj_institucion.get_codigo(
                            cargo_pasivo["id_institucion"])

                        # Si institucion no existe entonces Insert Institucion
                        if obj_institucion.codigo_institucion is None:
                            institucion, time_start, flag = functions.get_request_api(
                                url_api_list_instituciones.format(obj_institucion.id_institucion), time_start, header)
                            cantidad_total_de_peticiones_establecidos_en_la_API -= 1
                            obj_institucion.codigo_institucion = institucion["codigo"]
                            # Load a lista instituciones
                            obj_institucion.list_instituciones.append(
                                [obj_institucion.id_institucion, obj_institucion.codigo_institucion])
                            # Insert data
                            store_procedure = "EXEC [Ley_Lobby].[dbo].[ins_Institucion_sp] ?,?,?;"
                            params = (obj_institucion.id_institucion,
                                      obj_institucion.codigo_institucion, institucion["nombre"].strip())
                            crsr.execute(store_procedure, params)

                        # Insert Persona Pasiva
                        store_procedure = " EXEC [Ley_Lobby].[dbo].[ins_Perfil_sp] ?,?,?,?,?,?,?;"
                        params = (obj_persona_pasiva.nombres, obj_persona_pasiva.apellidos, 'Pasivo', obj_persona_pasiva._id_sujeto_pasivo_api,
                                  obj_cargo.nombre_cargo, obj_institucion.id_institucion, obj_cargo._id_cargo_api)
                        crsr.execute(store_procedure, params)
                        ...
                        crsr.execute(store_procedure, [
                                     id_perfil_temp, obj_cargo.fecha_inicio, obj_cargo.fecha_termino, obj_cargo.resolucion, obj_cargo.url_resolucion])
                        ...
                        if len(obj_cargo.list_identificadores_vinculados) > 0:
                            for identificador in obj_cargo.list_identificadores_vinculados:
                                crsr.execute("EXEC [Ley_Lobby].[dbo].[ins_Perfil_Has_Identificador_sp] ?,?;", [
                                             id_perfil_temp, identificador])
                    ...
                    if obj_persona_pasiva.id_perfil is None:
                        if cod_institucion is None:
                            obj_institucion.get_codigo(
                                audiencia["id_institucion"])
                        else:
                            obj_institucion.id_institucion, obj_institucion.codigo_institucion = audiencia[
                                "id_institucion"], cod_institucion

                        obj_persona_pasiva.id_perfil = next(
                            cargo[0] for cargo in list_cargos_pasivos_db if obj_institucion.id_institucion == cargo[1] and cargo[2] == audiencia['id_cargo'])

                    # Load Audiencia
                    # (self,id_audiencia,lugar,observacion,ubicacion,id_cargo):
                    obj_audiencia = controller.Audiencia(
                        audiencia["id_audiencia"], audiencia['lugar'], audiencia['referencia'], audiencia['comuna'], audiencia['fecha_inicio'], audiencia['fecha_termino'])

                    # Detalle de Audiencia API
                    detalle_audiencia, time_start, flag = functions.get_request_api(
                        url_api_audiencia.format(obj_audiencia.id_audiencia), time_start, header)
                    cantidad_total_de_peticiones_establecidos_en_la_API -= 1

                    # Create url's infoLobby & LeyLobby
                    obj_audiencia.url_build_web(
                        obj_institucion.codigo_institucion, obj_persona_pasiva._id_sujeto_pasivo_api)

                    # Detalle de Audiencia INSERT
                    store_procedure = "EXEC [Ley_Lobby].[dbo].[ins_Audiencia_sp] ?,?,?,?,?,?,?,?,?;"
                    crsr.execute(store_procedure, [obj_audiencia.id_audiencia, obj_audiencia.fecha_inicio, obj_audiencia.fecha_termino, obj_audiencia.lugar,
                                 detalle_audiencia['forma'], obj_audiencia.observacion, obj_audiencia.ubicacion, obj_audiencia.url_info_lobby, obj_audiencia.url_ley_lobby])
                    store_procedure = "EXEC [Ley_Lobby].[dbo].[ins_Perfil_Has_Audiencia_sp] ?,?,?,?,?;"
                    # Existen reuniones donde hay mas de dos integrantes que son pasivos (empleados publicos). Estos, solo estan en visibles en la direccion url (Pagina WEB) pero no dentro de la respuesta de la API
                    crsr.execute(store_procedure, [
                                 obj_audiencia.id_audiencia, obj_persona_pasiva.id_perfil, None, None, 'Principal'])
                    ...
                    for cargo_activo in detalle_audiencia['asistentes']:
                        obj_persona_activa = controller.Persona(
                            cargo_activo['nombres'], cargo_activo['apellidos'])
                        store_procedure = "EXEC [Ley_Lobby].[dbo].[ins_Perfil_sp] ?,?,?,?,?,?;"
                        crsr.execute(store_procedure, [
                                     obj_persona_activa.nombres, obj_persona_activa.apellidos, 'Activo', None, None, None])
                        ...
                        if 'rut_representado' in cargo_activo['representa']:
                            ...
                            crsr.execute(store_procedure, [obj_entidad.rut, obj_entidad.rut_es_valido, obj_entidad.nombre, obj_entidad.giro,
                                         obj_entidad.representante_directorio, obj_entidad.directorio, representa['pais'], obj_entidad.domicilio, obj_entidad.naturaleza])
                            ...
                            if functions.es_vacio_o_nulo(nombre) is False and any(char.isalpha() for char in nombre) and len(nombre) >= 3:
                                nombre_representa = functions.limpiar_texto(
                                    nombre)
                                nombre_representa = functions.limpiar_nombre(
                                    nombre_representa).title()
                                list_nombres_representa = nombre_representa.split()
                                list_nombres_persona = obj_persona_activa.nombres.split() + \
                                    obj_persona_activa.apellidos.split()
                                if any(nombre for nombre in list_nombres_persona if nombre in list_nombres_representa):
                                    nombre_representa = None

                        store_procedure = "EXEC [Ley_Lobby].[dbo].[ins_Perfil_Has_Audiencia_sp] ?,?,?,?,?;"
                        crsr.execute(store_procedure, [
                                     obj_audiencia.id_audiencia, obj_persona_activa.id_perfil, id_entidad, nombre_representa, None])

                    print(
                        f'id_audiencia: {id_audiencia} - cantidad de cargos activos: {len( detalle_audiencia["asistentes"] )} - cantidad de tiempo: {time.time()-start_performance}')
                ...
                if obj_num_page.num_page_incremento < list_audiencias_api["last_page"]:
                    ...
                    crsr.execute(
                        statement, [obj_num_page.num_page, "AUDIENCIA"])
