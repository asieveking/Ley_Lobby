...
with SQL_Conection.SQLServer() as crsr: 
    ...
    url_api_audiencia=Funciones.url_build_ley_lobby("Audiencia")
    
    while True:       
        try:
            list_audiencias_api,cantidad_total_de_peticiones_establecidos_en_la_API,time_start=Funciones.get_api(url_api_list_page.format(num_page),time_start,cantidad_total_de_peticiones_establecidos_en_la_API,header,True,2)
            ...
            for audiencia in list_audiencias_api["data"]:
                ...
                list_cargos_pasivos_api,cantidad_total_de_peticiones_establecidos_en_la_API,time_start=Funciones.get_api(url_api_list_cargos_pasivos.format(obj_persona_pasiva._id_sujeto_pasivo_api),time_start,cantidad_total_de_peticiones_establecidos_en_la_API,header,False,2)                
                ...
                for cargo_pasivo in list_cargos_pasivos_api: 
                    ...
                    if obj_institucion._cod_institucion  is None:                                                 
                        institucion,cantidad_total_de_peticiones_establecidos_en_la_API,time_start=Funciones.get_api(url_api_list_instituciones.format(obj_institucion._id_institucion),time_start,cantidad_total_de_peticiones_establecidos_en_la_API,header,False,2)
                    ...
                    if len(obj_cargo._list_identificadores_vinculados)>0:   
                        for identificador in obj_cargo._list_identificadores_vinculados:                  
                            crsr.execute("EXEC [Ley_Lobby].[dbo].[ins_Detalle_Perfil_Has_Identificador_sp] ?,?;",[id_perfil_temp,identificador])
                ...
                detalle_audiencia,cantidad_total_de_peticiones_establecidos_en_la_API,time_start=Funciones.get_api(url_api_audiencia.format(obj_audiencia._id_audiencia),time_start,cantidad_total_de_peticiones_establecidos_en_la_API,header,False,2)                       
